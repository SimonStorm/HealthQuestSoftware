...
#
# When a collection of files is about to be uploaded, Thin Putter will
...
##
# Copyright Rad Inks (Pvt) Ltd. 2005.
...
if($query->request_method() eq 'PUT')
{
	$fname = $query->path_info();
	carp "fname = $fname";
	$fname =~ s/$query->script_name()//g;
}
else
{
	$fname = $params{'fname'};
...
{
	carp "userfile_parent = $userfile_parent";
		...
		if($_ ne '.' && $_ ne '..')
		{
			...
			else
			{
				if($move == 1)
				{
					...
					#carp "Trying to move from $rel_path to $newPath$_";
				}
				print "<tr><td>$_</td><td>";
				print (-s ($newPath . $_));
				print "</td></tr>";
...
if(defined ($cmd))
{
	...
	elsif($cmd == 1)
	{
		...
		{
			carp "Sending filesize " . (-s "{$save_path}{$hash}/{$fname}");
			print -s "{$save_path}{$hash}/{$fname}";
		}
		else
		{
			print "00\n";
			carp "No resumption needed for" . "{$save_path}{$hash}/{$fname}";
		}
	}
	elsif($cmd == 2)
	{
...
else
{
	...
	{
		sysopen(FH, $fname, O_RDWR) or die "can't open numfile: $!";
		seek(FH,$offset,0);
	}
	else
	{
		sysopen(FH, $fname, O_RDWR|O_CREAT) or die "can't open numfile: $!";
	}
