...
require_once('files/register_funcs.inc');
require_once('files/login_funcs.inc');
require_once('files/Database.php');

$title = "Health Quest Software";// Title (may contain HTML tags)
...
$uploadmaxsize = "3000M";          // Max file size for uploads (check your php.ini)
...
$profile = array(
   "full" => array(
      ...
      "allowupdprofile" => true,           // Allows optional use of profile update screen
      "allowviewrpts" => true              // Allows optional access to usage logs
   ...
   "readonly" => array(
      ...
      "allowsearch" => true,               // Allows searches
      "allowregexpsearch" => true,         // Allows optional use of regular expressions in searches
      "allowupdprofile" => false,          // Allows optional use of profile update screen
      "allowviewrpts" => false             // Allows optional access to usage logs
   ),
   "client" => array(
      "allowmove" => false,                // Allows file and directory moving
      "allowrename" => false,              // Allows file and directory renaming
...
$user = array(
   "admin" => array(
      "password" => "password",
      "profile" => "admin",
      "folders" => array("Auto Account", "Health Leap", "LeapTo", "Market Leap", "PO Leap", "uploads")
   ),
   "user" => array(
      "password" => "userpwd",
      "profile" => "readonly"
   ...
   "simon" => array(
      "password" => "password",
      "profile" => "client",
      "folders" => array("Auto Account", "Health Leap")
   ),
   "bob" => array(
      "password" => "password",
      "profile" => "client",
      "folders" => array("Auto Account", "LeapTo")
   )
);
...
//SPS CHANGED THIS TO GET RID OF FILE LINK

$filelinks = false;                 // Links on files (inhibited with a custom $basedir
...
$messages = array(
   ...
   "edt8"=>"Edit file",
   "edt9"=>"View file",
   "edt10"=>"Edit", // E(dit action)
   "edt11"=>"View", // V(iew action)
...
if (ISSET($_POST['downfile'])) {
	// INSERT INTO LOG
	$LogQuery = "INSERT INTO userlog (user_name, role, remote_addr, action, details)
			VALUES ('".$_SESSION['username']."', ".$_SESSION['Role'].", '".$_SERVER['REMOTE_ADDR']."', 'Download', '".$_POST['downfile']."')";
...
// 20110823 - SPS - added this to try to fix .mdb download issue
//else
//{
...
$allowupdprofile = $profile[$userprofile]["allowupdprofile"];
$allowviewrpts = $profile[$userprofile]["allowviewrpts"];
...
//   echo '</pre>';
	...
	// INSERT INTO LOG
	$LogQuery = "INSERT INTO userlog (user_name, role, remote_addr, action, details)
			VALUES ('".$_SESSION['username']."', ".$_SESSION['Role'].", '".$_SERVER['REMOTE_ADDR']."', 'Download', '".@basename($file)."')";
...
// File list page
   ...
   if (!empty($files)) {
      ...
      echo "<input name=downfile type=hidden value=''>";
      echo "<input name=subdir type=hidden value=\"$subdir\">";
      echo "<input name=sortby type=hidden value=$sortby>";
      echo "<tr>";
      ...
      while (list($key, $file) = each($files)) {
         // Directory section
         if ($file["dir"]) {
            // SPS - This is where we add folder level security
            if (($subdir != "") || ($file["name"] != "..")) {
				   if (($subdir != "")) {
					   echo "<tr>";
			...
			$query = "SELECT AutoAccount, HealthLeap, LeapTo, MarketLeap, POLeap, uploads
					  FROM user u, SoftwareAccess s
					  WHERE u.user_id = s.ClientId
			...
			if (!$result || mysql_num_rows($result) < 1) {
			  ...
			  return $feedback;
			   ...
			   $AutoAccount = mysql_result($result, 0, 'AutoAccount');
			   $HealthLeap = mysql_result($result, 0, 'HealthLeap');	       
			   $LeapTo = mysql_result($result, 0, 'LeapTo');	       	       
			   $MarketLeap = mysql_result($result, 0, 'MarketLeap');	       	       
			   $POLeap = mysql_result($result, 0, 'POLeap');	  
			   $uploads = mysql_result($result, 0, 'uploads');	  
			   if (($file["name"] == 'Auto Account' && $AutoAccount == 'Y')
			   		|| ($file["name"] == 'Health Leap' && $HealthLeap == 'Y')
			   		|| ($file["name"] == 'LeapTo' && $LeapTo == 'Y')
			   		|| ($file["name"] == 'Market Leap' && $MarketLeap == 'Y')
			   		|| ($file["name"] == 'PO Leap' && $POLeap == 'Y')
			   		|| ($file["name"] == 'uploads' && $uploads == 'Y')) {
				   echo "<tr>";
				   if (($file["level"] == 9) && ($allowmove || $allowrename || $allowdelete)) {
					  echo "<td><input type=checkbox name=\"selfiles[]\" value='".$file["name"]."'></td>";
				   } else {
...
// SPS Add Onclick to Log               
               echo " target=\"_new\" onMouseOver='return statusMsg(\"".quoteJS($file["statusmsg"])."\");' onMouseOut='return statusMsg(\"\");' onClick='return logaction(\"".$file["name"]."\");'>";
               echo $file["alias"];
               echo "</a>";
            } else {
            ...
            if (($allowdownload) && ($subdir != $trashcan)) {
               echo "<a href=$thisscript?act=download&subdir=".rawurlencode($subdir)."&sortby=$sortby&file=".str_replace("%2F", "/", rawurlencode(extractSubdir($file["path"])))." onClick='return logaction(\"".$file["name"]."\"); onMouseOver='return statusMsg(\"".quoteJS($messages["dwn5"])."\");' onMouseOut='return statusMsg(\"\");'>".(($useimages) ? "<img src=\"$imagesdir/$downloadimage\" border=0>" : $messages["dwn1"])."</a> ";
            }
            echo "</td>";
            echo "</tr>";
...
// SPS
         if (   ($act != "search")
             && $allowupload && $subdir == 'uploads') {
            echo "<form action=\"/cgi-bin/upload.cgi\" method=\"POST\" ENCTYPE=\"multipart/form-data\">";
...
//            echo "<input name=max_file_size type=hidden value=$uploadmaxsize>";
            ...
            echo "</td>";
            echo "<td class=tdlt colspan=".($nbcols - 3).">";
            echo "<input name=FILE1 type=FILE size=15> ";
            echo "<input type=submit value=\"".$messages["fup0"]."\">";
            echo "</td>";
            echo "</tr>";
            echo "</form>";
         }
...
// SPS Update user profile link         
         if (   ($act != "search")
             && $allowupdprofile) {
            echo "<tr>";
            echo "<td class=tdcc colspan=".($nbcols).">";
            echo "</td>";
            echo "</tr>";
            echo "<tr>";
            echo "<td class=tdcc colspan=".($nbcols).">";
            echo "<a href=files/useraccess.php>Update User Profiles</a>";
            echo "</td>";
            echo "</tr>";
         }
         if (   ($act != "search")
            ...
            echo "<tr>";
            echo "<td class=tdcc colspan=".($nbcols).">";
            echo "<a href=files/userlogs.php>View Application Usage</a>";
            echo "</td>";
            echo "</tr>";
         }
